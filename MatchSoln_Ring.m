...
q=qfunc(1/4, -1, 0, sqrt(mu)*r(length(r)),length(r));
% Q=[q(1)*ones(floor(N/100),1);q];
Q=[q];
for j=1:N+1
% u(1+(j-1)*L:j*L) = a_out(j).*r.*besselj(k*(j-1)+1,r).*exp(-sqrt(mu)*r);
u(1+(j-1)*L:j*L) = a_out(j).*r.*besselj(k*(j-1)+1,r).*Q;
end
...
colormap(z1,newmap);
  ...
  z2 = figure('Position',[7*scrsz(3)/12 scrsz(4)/4 scrsz(3)/3 scrsz(4)/2]);
  plot((0:N)/(N), a_out*(N));
  ylim([0.4 0.7]);
   grid on;
   hold off;
