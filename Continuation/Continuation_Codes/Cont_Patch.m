...
c0=1/4; c3=-((19*p(2)^2/18)+(3/4)*p(3));
% q=qfunc(c0, -1, sqrt(p(1))*r(floor(N/100)), sqrt(p(1))*r(N),N-floor(N/100));
% q=qfunc(c0, -1, 0, sqrt(p(1))*r(N),N);
q=qfunc(1/4, -1, 0, sqrt(p(1))*r(N),N);
% q = q/sqrt(abs(c3));
% Q=[q(1)*ones(floor(N/100),1);q];
Q=[q];
% figure;plot(r,Q)
% pause; close all
...
if p(5) == 1
compSpec = @(u,p) ComputeEigenvalues_Rad(u,p,prob,mesh_params);
